  ...
  return <div className="flex min-h-screen flex-col bg-neutral">
    <header className="site-header sticky top-0 z-40 border-b border-[#E2E8F0]">
      <div className="container-custom flex h-16 items-center justify-between gap-6">
        <Link to="/" className="flex items-center gap-2 text-primary" onClick={close}>
          <img src="/logo.png" alt="BookMySpace" className="h-8 w-auto" />
          <span className="brand-mark text-sm font-bold">BOOKMYSPACE</span>
        </Link>
    ...
    <footer className="bg-primary text-white">
      <div className="container-custom grid gap-10 py-14 md:grid-cols-[1.7fr_1fr_1fr]">
        <div><div className="mb-5 flex items-center gap-2"><img src="/logo.png" alt="BookMySpace" className="h-8 w-auto brightness-0 invert" /><span className="brand-mark text-sm font-bold">BOOKMYSPACE</span></div><p className="max-w-sm text-sm leading-6 text-white/70">Your trusted partner in finding the perfect space. We verify listings so every move begins with confidence.</p><div className="mt-6 flex gap-2"><span className="grid h-8 w-8 place-items-center rounded-full bg-white/10 text-xs">in</span><span className="grid h-8 w-8 place-items-center rounded-full bg-white/10 text-xs">ig</span></div></div>

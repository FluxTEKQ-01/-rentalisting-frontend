...
export default function Home() {
  ...
  const { data, isLoading, error } = useQuery({ queryKey: ['recent-approved-rentals'], queryFn: () => propertyApi.list({ status: 'published', limit: '3', sort: 'latest' }) });
              ...
              <article key={property._id} className="card-hover flex h-full flex-col">
                <Link to={`/properties/${property._id}`} className="relative block aspect-[16/10] overflow-hidden bg-[#E2E8F0]">
                  {property.images?.[0]?.url ? (
                    <img loading="lazy" className="h-full w-full object-cover transition duration-500 hover:scale-105" src={property.images[0].url} alt={property.title} />
                  ) : (
